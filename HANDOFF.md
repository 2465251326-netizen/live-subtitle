...
> 最后更新：2026-09-11 v2.3.6 发布后（P7 译文修正词典 + P8 指引电平守卫 + 测试套件挂死根治）
...
- **当前版本**：**v2.3.6**（已发布，含 Setup EXE + portable zip 双资产）
...
python tests/test_units.py                   # 33 项单元测试
python tests/test_integration.py             # 33 项集成测试（v2.3.6 起含 P7/P8；套件带看门狗与 os._exit）
...
- 集成测试：`tests/test_integration.py`（33 项，覆盖配置/缓存/重采样/悬浮条三模式/字幕卡生命周期/热键/设置字段/QSS 括号/向导/退出清理/声明式行表/SRT/呼吸与贴边等体验回归）
...
tests/test_units.py      33 项单元测试
tests/test_integration.py 33 项集成测试
