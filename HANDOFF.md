...
> 最后更新：2026-09-12 v2.4.4 发布后（摸底轮九项修复：向导不弹/GPU静默回落/告警撤回/暂存丢失/纠错菜单可达/SRT默认/引导复位/路径/列表保留；摸底报告见工作区 BUG-ROUNDUP-20260912.md）
...
- **当前版本**：**v2.4.4**（已发布，含 Setup EXE + portable zip 双资产）
...
python tests/test_integration.py             # 59 项集成测试（判定以 test_report.txt 的 TOTAL 行为准，退出码有 Qt 收尾竞态噪声）
...
- 集成测试：`tests/test_integration.py`（59 项，覆盖配置/缓存/重采样/字幕面板(成对行·淘汰·清空·收起·拖移契约·空状态·图钉·引导·未读计数·高度收敛)/字幕卡生命周期/热键/设置字段/QSS 括号/向导/退出清理/声明式行表/SRT/呼吸与贴边等体验回归）
...
tests/test_integration.py 59 项集成测试
