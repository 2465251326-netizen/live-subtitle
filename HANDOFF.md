...
> 最后更新：2026-09-11 v2.3.18 发布后（P23 攒句绝对寿命 10s 封顶；四十九章 8s 结论已勘误）
...
- **当前版本**：**v2.3.18**（已发布，含 Setup EXE + portable zip 双资产）
...
python tests/test_integration.py             # 39 项集成测试（v2.3.8 起含 P9 两轨制；判定以 test_report.txt 的 TOTAL 行为准，退出码有 Qt 收尾竞态噪声）
...
- 集成测试：`tests/test_integration.py`（39 项，覆盖配置/缓存/重采样/悬浮条三模式/字幕卡生命周期/热键/设置字段/QSS 括号/向导/退出清理/声明式行表/SRT/呼吸与贴边等体验回归）
...
tests/test_integration.py 39 项集成测试
