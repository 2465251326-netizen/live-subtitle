...
> 最后更新：2026-09-18 **v2.20.0 已发布**（第二十九节＝本轮：上下双语按用户实拍重做——历史区与滚动字幕墙两态删除、大字分栏成为唯一形态；面板 ⋯ 菜单新增攒句开关；流式合并再削一条复读 + 自查发现单字符重叠吃字）。同日更早：v2.19.4（使用体验巡检）见第二十八节、v2.19.3（流式复读三根因）见第二十七节、v2.19.2（面板透明真根因 + 四路巡检）见第二十六节；更早同日还有 v2.19.1（发版收尾 + 令牌轮换 + 视觉能力实测，第二十五节）。⚠ 第二十四节 24.7 的"滚动字幕墙"设计已被本轮改判删除，读那段前先看段首警示。再上一轮记录（2026-09-16 **v2.18.2 已发布**，本行下面首段仍描述 v2.18.1）——面板"只留一条分割线"其实没做到（v2.18.0 漏删历史区装饰线 + 拖完胶囊永久高亮），另修字幕卡聚焦样式自 v2.2.5 起从未生效等 8 项，以及**真实英语新闻端到端实测揪出的 5 项悬浮窗缺陷**，见 CHANGELOG 与第二十/二十一节；**同日本会话另实测出三个真缺陷并全部修复补锁**——D-1 流式原文在 `asr_language=auto`（出厂默认）下整条通道静默哑火、D-2 dual 布局延续片段原文被拼接两遍、D-3 首句草稿译文必错一轮，见第二十二节（含 22.8 发版前真机复测），**v2.18.2 已发布**
...
- **当前版本**：**v2.20.0**（已发布，含 Setup EXE + portable zip 双资产）
