...
> 最后更新：2026-09-11 v2.3.8 发布后（第五轮体验 P9/P10 落地；攒句判据以实测修正为大小写边界）
...
- **当前版本**：**v2.3.8**（已发布，含 Setup EXE + portable zip 双资产）
...
python tests/test_units.py                   # 35 项单元测试
python tests/test_integration.py             # 34 项集成测试（v2.3.8 起含 P9 两轨制；判定以 test_report.txt 的 TOTAL 行为准，退出码有 Qt 收尾竞态噪声）
...
- 集成测试：`tests/test_integration.py`（34 项，覆盖配置/缓存/重采样/悬浮条三模式/字幕卡生命周期/热键/设置字段/QSS 括号/向导/退出清理/声明式行表/SRT/呼吸与贴边等体验回归）
...
tests/test_units.py      35 项单元测试
tests/test_integration.py 34 项集成测试
...
- **已完成**：两轮 E2E → v2.2.11（六修复+SRT 导出）→ v2.2.12（硬件预选/全屏隐藏开关/聆听呼吸/SRT 折行/CI pull_request）→ v2.2.13（用户实拍速览卡版式修复+探测器盲区补强）→ v2.2.14（用户裁决：删全屏隐藏+下载 ETA）→ v2.3.0（声明式设置框架迁移，行为不变）。提交链：`2326886` → `bc2d3a9` → `8c0a553` → `d5e77ee` → `343f583` → `3821b7c` → `aeb2fb1`(v2.3.2 G1/G2) → `fe12795`(v2.3.3 P1/P2) → `9e07c3d`(v2.3.4 碎片漏网修) → `ab5a0af`(v2.3.5 P5 预热) → `c36c9cf`(v2.3.6 P7/P8+套件根治) → `5bcf032`(v2.3.7) → `25d9e35`(v2.3.8 攒句判据修正)，各版 tag CI success、双资产核对在位
