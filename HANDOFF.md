...
> 最后更新：2026-09-11 v2.3.5 发布后（P5 启动预热落地：按J到就绪 37.5s→0.013s 实测）
...
- **当前版本**：**v2.3.5**（已发布，含 Setup EXE + portable zip 双资产）
...
python tests/test_integration.py             # 31 项集成测试（v2.3.5 起含 P5 预热安全边界）
...
- 集成测试：`tests/test_integration.py`（31 项，覆盖配置/缓存/重采样/悬浮条三模式/字幕卡生命周期/热键/设置字段/QSS 括号/向导/退出清理/声明式行表/SRT/呼吸与贴边等体验回归）
...
tests/test_integration.py 31 项集成测试
