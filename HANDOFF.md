...
> 最后更新：2026-09-11 v2.3.2 发布后（体验报告缺口 G1/G2 清零）
...
- **当前版本**：**v2.3.2**（已发布，含 Setup EXE + portable zip 双资产）
...
python tests/test_integration.py             # 29 项集成测试（v2.3.2 起含 G1/G2 体验缺口回归）
...
tests/test_units.py      30 项单元测试
tests/test_integration.py 29 项集成测试
...
- **已完成**：两轮 E2E → v2.2.11（六修复+SRT 导出）→ v2.2.12（硬件预选/全屏隐藏开关/聆听呼吸/SRT 折行/CI pull_request）→ v2.2.13（用户实拍速览卡版式修复+探测器盲区补强）→ v2.2.14（用户裁决：删全屏隐藏+下载 ETA）→ v2.3.0（声明式设置框架迁移，行为不变）。提交链：`2326886` → `bc2d3a9` → `8c0a553` → `d5e77ee` → `343f583` → `3821b7c` → `aeb2fb1`(v2.3.2 G1/G2)，各版 tag CI success、双资产核对在位
