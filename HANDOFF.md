...
> 最后更新：2026-09-12 v2.4.3 发布后（面板增强 A~E 全量落地 + 高度贴内容修复：sizeHint 未定型→延迟收敛复排）
...
- **当前版本**：**v2.4.3**（已发布，含 Setup EXE + portable zip 双资产）
...
python tests/test_integration.py             # 51 项集成测试（判定以 test_report.txt 的 TOTAL 行为准，退出码有 Qt 收尾竞态噪声）
...
- 集成测试：`tests/test_integration.py`（51 项，覆盖配置/缓存/重采样/字幕面板(成对行·淘汰·清空·收起·拖移契约·空状态·图钉·引导·未读计数·高度收敛)/字幕卡生命周期/热键/设置字段/QSS 括号/向导/退出清理/声明式行表/SRT/呼吸与贴边等体验回归）
...
tests/test_integration.py 51 项集成测试
