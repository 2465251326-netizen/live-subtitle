...
DEFAULTS = {
    ...
    "overlay_layout": "list",          # v2.11.0：字幕面板布局——"list"=历史滚动列表
                                        ...
                                        # 面板 ⋯ 菜单随时切换，设置页保存即时生效。
    "overlay_dual_hist": True,         # v2.14.0：dual 布局的历史区开关。
                                        # v2.19.0 曾按首轮实拍裁决改为默认关（当时历史区
                                        # 几何有 BUG 吃掉 2/3 面板）；v2.19.1 用户真机
                                        # 二轮裁决**恢复默认开**："句子全部保留，不能在
                                        # 字幕悬浮窗里消失"——几何已在 23.2 修正（历史区
                                        # 按内容收缩、当前句保底 92px），首轮否掉的形态
                                        # 成因已不存在。关=当前句独占面板（能力保留，
                                        # 设置页与面板 ⋯ 菜单均可切换，重启保持）。
    ...
    "translate_grouping": True,        # v2.19.0：翻译侧"攒句合并"开关（默认开=现状两轨制：
                                        # 上屏碎、翻译按整句送，译文更连贯）。
                                        # 关=**每个识别片段一到达就立刻送翻译**（逐片实时）；
                                        # 代价如实写进设置页：跨片语义被切断、机翻味更重、
                                        # 在线引擎请求量上升（离线包无额度压力）。
                                        # 与 low_latency_mode 相互独立——关攒句**不会**把分段
                                        # 退回 14s 慢档（那是旧"低延迟关"分支的副作用）。
