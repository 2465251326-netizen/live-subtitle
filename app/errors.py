...
_PATTERNS = [
    ...
    # v2.0.9：401/403 鉴权失败——本机存有过期 HF 令牌时公开仓库也报 401；
    # 匹配 requests 的标准错误文本避免误伤普通路径/数字
    (r"401 Client Error|403 Client Error|Unauthorized for url",
     "模型下载源拒绝访问（本机可能存有过期/无效的 Hugging Face 令牌）"),
...
_ACTIONS = {
    ...
    "模型下载源拒绝访问（本机可能存有过期/无效的 Hugging Face 令牌）":
        "v2.0.9 起下载已强制匿名；若旧版本仍报此错请升级，或删除 %USERPROFILE%\\.cache\\huggingface\\token 后重试",
