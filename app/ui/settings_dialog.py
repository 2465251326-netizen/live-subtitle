...
MODELS = [("tiny", ui_text("tiny · 最快 · 延迟约 2s")),
          ("base", ui_text("base · 流畅 · 中文较弱")),
          ("small", ui_text("small · 推荐（4 核以上）")),
          ("medium", ui_text("medium · 高精度 · 需好 CPU")),
          ("large-v3-turbo", ui_text("large-v3-turbo · 顶级精度 · 需 GPU/高配 CPU（约 6s+）"))]
...
class ModelDownloadWorker(QThread):
    ...
    def run(self):
        ...
        if AsrThread.model_cached(self.model_size):
            self.finished_ok.emit(f"{self.model_size}{ui_text(' 模型已就绪')}")
            return
        try:
        ...
        def _progress(i, total, name):
            self.progress_text.emit(f"{ui_text('正在下载模型文件（')}{i}/{total}）：{name}")
        ...
        try:
            ...
            if self._cancel:
                ...
                self.finished_ok.emit(ui_text("下载已取消（已下载部分保留，下次继续）"))
                return
            from app.errors import friendly_error
            self.failed.emit(f"{ui_text('模型下载失败：')}{friendly_error(e)}")
            return
        ...
        self.finished_ok.emit(f"{self.model_size}{ui_text(' 模型下载完成——下次开始翻译即离线可用')}")
...
_STD_ROW_ITEMS = {
    "asr_accuracy": [(ui_text("快速（推荐，实时字幕）"), "fast"),
                     (ui_text("均衡（更准，略慢）"), "balanced"),
                     (ui_text("高精度（最准，明显更慢，适合整理字幕）"), "quality")],
    "asr_lang": [(ui_text("自动检测"), "auto")] + [
        (name, code) for code, name in LANGUAGES.items()
        if code not in ("zh-CN", "zh-TW", "auto")],
    # v2.3.0 顺带修正：原手写版循环未排除 "auto"，下拉里"自动检测"出现两次
    "compute": [(ui_text("CPU 模式（通用）"), "cpu"),
                (ui_text("强制 GPU（需先装好 CUDA 运行时）"), "cuda"),
    ...
    "close": [(ui_text("每次询问"), "ask"), (ui_text("隐藏到托盘（字幕继续）"), "tray"),
              (ui_text("直接退出程序"), "exit")],
    # v2.11.0：面板布局两形态（dual=上下双语豆包风）；v2.20.1：双语两栏逐句累积
    "overlay_layout": [(ui_text("列表历史（原文+译文成对滚动、可回看整场）"), "list"),
                       (ui_text("上下双语（豆包风：上原文 / 下译文卡片、两栏逐句累积）"), "dual")],
    # v2.7.6（C）：分段上限档位——实测 hold_p50 恒等于该值（turbo 关=6.04s、
    # turbo 开=4.03~4.43s）。开了推测式增量翻译后译文随碎片立即上屏，本项
    # 主要影响"刷新粒度与腰斩程度"，不再是译文迟到的决定因素
    "segment_cap": [(ui_text("2.5 秒（刷新最密，句子最易被腰斩）"), 2.5),
                    (ui_text("3 秒"), 3.0),
                    (ui_text("4 秒（默认，= 榨干模式档）"), 4.0),
                    (ui_text("6 秒（低延迟模式旧默认）"), 6.0),
                    (ui_text("10 秒（整句优先，明显更慢）"), 10.0),
                    (ui_text("0 = 跟随模式默认"), 0.0)],
}

_STD_ROWS = [
    {"key": "asr_language", "attr": "asr_lang_combo", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "combo", "title": ui_text("识别语言"),
     "desc": ui_text("「自动检测」会在第一句话后锁定说话语言，并配合下方「语言复检」定期纠偏；已知语言时手动锁定更快更稳。"),
     "opts": {"items": _STD_ROW_ITEMS["asr_lang"]}},
    {"key": "asr_device", "attr": "compute_combo", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "combo", "title": ui_text("计算方式"),
     "desc": ui_text("「强制 GPU」= 只用 NVIDIA 显卡加速（需先「检测 GPU 环境」并安装 CUDA 版 PyTorch；"
             "不可用时自动回落 CPU 并在就绪提示中说明原因）。"
             "「自动」= 稳定优先的 CPU 模式。无独显或打包版保持 CPU 即可实时。"),
     "opts": {"items": _STD_ROW_ITEMS["compute"]}},
    {"key": "hallucination_filter", "attr": "hallucination_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("幻觉抑制"),
     "desc": ui_text("自动丢弃音乐/噪声段的胡言乱语字幕（推荐开启；若发现正常语音被误丢可关闭）。")},
    {"key": "asr_accuracy", "attr": "accuracy_combo", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "combo", "title": ui_text("识别精度"),
     "desc": ui_text("快速=实时字幕推荐（识别束宽最窄）；均衡/高精度识别更准但明显更慢（高精度还会结合前文语境），适合事后整理字幕。"
             "实时看视频请保持快速——准确度问题优先用下方「误听修正词典」定点纠正。"),
     "opts": {"items": _STD_ROW_ITEMS["asr_accuracy"]}},
    {"key": "silero_vad", "attr": "silero_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("Silero VAD（段内净化）"),
     "desc": ui_text("在识别前用 Silero 模型过滤段内非语音（背景音乐/噪声更干净），与切句 VAD 双保险。")},
    {"key": "low_latency_mode", "attr": "low_latency_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("低延迟模式（直播/新闻推荐）"),
     "desc": ui_text("字幕更快上屏（看视频强烈推荐）：静音判停收紧 0.45 秒→0.30 秒，连续说话时字幕不再攒十几秒才出。"
             "分段上限也从 14 秒收到 6 秒——「注意」：下方「连续语流分段上限」一旦设成具体秒数就以该项为准"
             "（出厂默认 4 秒），此时本项实际只改变静音判停。v2.3.7 起翻译自动攒整句"
             "（上屏快、译文仍是完整句子，不再半截话各翻各的）；显示上句子可能切短。"),
     "opts": {}},
    {"key": "early_flush", "attr": "early_flush_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("提前冲句（低延迟增强）"),
     "desc": ui_text("说完最后一句后的等待从 3.5 秒缩短到 2 秒即冲整句译文（静默判据不变，只是地板更低）。"
             "连续说话时译文节奏不受影响（仍按整句成批出）；若发现句子常被切短可关闭。仅低延迟模式生效。"),
     "opts": {}},
    {"key": "perf_turbo", "attr": "perf_turbo_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("榨干模式（硬件全速）"),
     "desc": ui_text("一个开关榨尽算力：GPU 推理权重压缩为 INT8（解码约快 1.2~1.6 倍、显存约省半，"
              "识别率可能轻微下降）＋翻译运行期进程优先级提升（后台其他程序相对变慢）＋"
              "连续说话切段上限 6s→4s＋冲句地板 2.0s→1.2s（句子更易切短）。追求极限速度再开；"
              "挂机求稳请保持关闭。GPU/优先级/切段上限重启翻译生效；冲句节奏保存后即时生效。"
              "注：切段上限若被下方「连续语流分段上限」设成具体秒数，则以该项为准。"),
     "opts": {}},
    # v2.7.6：延迟三件套（用户反馈"翻译速度还是太慢"——遥测定位识别 0.55s +
    # 翻译 0.06s 只占 13%，hold_p50≈4.1s 的攒句等待才是大头）
    {"key": "spec_translate", "attr": "spec_translate_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("推测式增量翻译（治译文迟到）"),
     "desc": ui_text("识别碎片一到达就立刻翻译「目前攒到的文本」并上屏，下一片到达再送更长版本，"
     ...
     "opts": {}},
    {"key": "translate_grouping", "attr": "grouping_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("翻译攒句合并（关掉＝逐片实时翻译）"),
     "desc": ui_text("开（默认）＝识别碎片照常逐片上屏，但翻译等整句攒完再送，译文更连贯"
     ...
     "opts": {}},
    {"key": "neural_vad", "attr": "neural_vad_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("神经 VAD 句末判定（实验性，默认关）"),
     "desc": ui_text("用 Silero 模型判定「这块是不是人声」来找句末停顿，代替能量判据。默认关闭的原因如实相告："
     ...
     "opts": {}},
    {"key": "stream_preview", "attr": "stream_preview_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("流式原文（上下双语增强）"),
     "desc": ui_text("「上下双语」布局下，每 0.9 秒把最近 4 秒音频重识别一次，把新增话音实时追加到原文区——"
     ...
     "opts": {}},
    {"key": "segment_cap_s", "attr": "segment_cap_combo", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "combo", "title": ui_text("连续语流分段上限"),
     "desc": ui_text("连续说话不停顿时，强制在这个长度切一刀交付一片。v2.7.5 实测「译文等待中位数」约等于这个数值"
    ...
    {"key": "lang_recheck", "attr": "lang_recheck_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("语言复检"),
     "desc": ui_text("自动检测下每 20 段临时解除语言锁定重听一次：视频中途换语言、或开头锁错语言时能自动纠正；"
             "复检结果与当前锁定不一致且不够可信时，维持原语言、该复检段保守丢弃不上屏。"),
     "opts": {}},
    {"key": "prewarm_model", "attr": "prewarm_check", "page": "asr", "section": ui_text("语言与计算"),
     "kind": "check", "title": ui_text("启动时预热模型"),
     "desc": ui_text("打开软件即在后台把已下载的模型加载好，点「开始翻译」几乎秒就绪——"
             "否则每次冷启动要等 GPU 初始化近 1 分钟。仅预热已下载模型，不会自动下载；关闭可省显存占用。"),
     "opts": {}},
    {"key": "engine", "attr": "engine_combo", "page": "translate", "section": ui_text("翻译方向"),
     "kind": "combo", "title": ui_text("翻译引擎"),
     "desc": ui_text("全部免费无需密钥。自动模式启动时探测在线接口并选用可达者、断网自动回退离线包；"
             "Argos 为完全离线方案，逐句直译、多义词易翻错（通顺度低于在线），需先在下方下载语言包。"),
     "opts": {"items": _STD_ROW_ITEMS["engine"], "on_change": "_on_engine_changed"}},
    {"key": "target_lang", "attr": "target_combo", "page": "translate", "section": ui_text("翻译方向"),
     "kind": "combo", "title": ui_text("翻译目标语言"),
     "desc": ui_text("在线引擎支持简繁中文、英、日、韩、法、德、西、俄、葡、意、泰、越、阿、印尼、印地共 16 种；"
             "离线语言包支持其中 15 种（暂缺繁体中文），选 Argos 引擎后可下载。"),
     "opts": {"items": _STD_ROW_ITEMS["target"], "on_change": "_on_engine_changed"}},
    {"key": "engine_auto_fallback", "attr": "engine_auto_fallback_check", "page": "translate", "section": ui_text("翻译方向"),
     "kind": "check", "title": ui_text("引擎自动切换（失败降级备援）"),
     "desc": ui_text("开启：当前翻译引擎失败时自动换备援通道（在线互备→已装离线包），网络恢复后自动切回。"
             "关闭：固定使用当前引擎，失败只在字幕卡与状态栏明确报错、绝不悄悄换通道——"
             "适合要求译文来源稳定（如对照测试、离线包用户）。重启翻译生效。"),
     "opts": {}},
    {"key": "offline_quality", "attr": "offline_quality_combo", "page": "translate", "section": ui_text("翻译方向"),
     "kind": "combo", "title": ui_text("离线翻译质量"),
     "desc": ui_text("仅影响 Argos 离线引擎（含自动模式断网回落）。高质量=更宽的翻译束宽，译文更连贯、"
             "直译痕迹更少，单句离线翻译耗时略增（约 0.1 秒，字幕上屏几乎无感）；快速=保持旧行为。"
             "在线翻译不受此项影响。"),
     "opts": {"items": _STD_ROW_ITEMS["offline_quality"]}},
    {"key": "fix_whole_word", "attr": "fix_whole_word_check", "page": "translate", "section": ui_text("译文质量"),
     "kind": "check", "title": ui_text("词典整词匹配"),
     "desc": ui_text("开启后英文词条只在整词出现时替换（strikes 不再误伤专名短语），多义词纠错更安全；"
    ...
    {"key": "show_source", "attr": "show_source_check", "page": "display", "section": ui_text("字幕显示"),
     "kind": "check", "title": ui_text("同时显示原文"),
     "desc": ui_text("开启后字幕与字幕面板同时保留原语言文本（面板上也可一键开关）。")},
    {"key": "overlay_layout", "attr": "layout_combo", "page": "display", "section": ui_text("字幕显示"),
     "kind": "combo", "title": ui_text("面板布局"),
     "desc": ui_text("「列表历史」= 原文+译文成对滚动、自动跟随最新，可回看整场。\n"
    ...
    {"key": "instant_caption", "attr": "instant_caption_check", "page": "display", "section": ui_text("上屏行为"),
     "kind": "check", "title": ui_text("字幕流式上屏（原文先出）"),
     "desc": ui_text("开启：识别文本立刻上屏（译文位置显示占位），译文就绪后原地补齐——听到哪看到哪。"
             "关闭：等识别+翻译都完成后一次性显示整条字幕（旧行为）。")},
    {"key": "close_action", "attr": "close_combo", "page": "general", "section": ui_text("窗口行为"),
     "kind": "combo", "title": ui_text("点击关闭按钮时"),
     "desc": ui_text("「隐藏到托盘」后主窗口消失，识别与翻译在后台继续，悬浮字幕正常显示，"
             "可从任务栏右下角托盘图标重新打开主窗口。"),
     "opts": {"items": _STD_ROW_ITEMS["close"]}},
    {"key": "auto_start", "attr": "auto_start_check", "page": "general", "section": ui_text("窗口行为"),
     "kind": "check", "title": ui_text("启动后自动开始翻译"),
     "desc": ui_text("打开软件后自动按上次配置开始识别与翻译，适合固定场景挂机使用。")},
    {"key": "max_history", "attr": "max_history_spin", "page": "general", "section": ui_text("字幕记录"),
     "kind": "spin", "title": ui_text("主窗口最多保留字幕条数"),
     "desc": ui_text("超出后自动清理最早的记录，避免长时间运行占用内存。"),
...
class _ModelDetailDialog(QDialog):
    ...
    def __init__(self, parent, code, current_model, running):
        ...
        self.setWindowTitle(f"{ui_text('模型详情 · ')}{code}")
        self.resize(480, 250)
        v = QVBoxLayout(self)
        title = QLabel(intro)
        ...
        btns = QHBoxLayout()
        self.dl_btn = QPushButton(ui_text("下载模型"))
        self.dl_btn.clicked.connect(self._start_download)
        self.cancel_btn = QPushButton(ui_text("取消下载"))
        self.cancel_btn.setVisible(False)
        self.cancel_btn.clicked.connect(self._cancel_download)
        self.rm_btn = QPushButton(ui_text("删除模型"))
        self.rm_btn.clicked.connect(self._remove)
        close_btn = QPushButton(ui_text("关闭"))
    ...
    def closeEvent(self, event):
        if self.worker is not None and self.worker.isRunning():
            event.ignore()
            QMessageBox.information(self, ui_text("下载进行中"),
                                    ui_text("模型正在下载，请先「取消下载」或等待完成后再关闭。"))
            return
        event.accept()
    ...
    def _refresh(self):
        ...
        text = {
            "missing": ui_text("状态：未下载（可点「下载模型」提前下载；首次使用也会自动下载）"),
            "partial": f"{ui_text('状态：下载不完整 · 已占 ')}{mb:.0f}{ui_text(' MB（残留半截文件，可删除或重新下载续传）')}",
            "full": f"{ui_text('状态：已下载 · ')}{mb:.0f} MB",
        }[state]
    ...
    def _start_download(self):
        ...
        self.dl_status.setText(ui_text("正在连接下载源（首次可能需探测镜像）..."))
    ...
    def _cancel_download(self):
        if self.worker is not None and self.worker.isRunning():
            self.worker.cancel()
            self.dl_status.setText(ui_text("正在取消（等待当前文件下载完成，最长可能数十秒）..."))
    ...
    def _remove(self):
        ...
        if self.code == self.current_model and self.running:
            QMessageBox.warning(self, ui_text("无法删除"),
                                ui_text("该模型正在使用中，请先停止翻译再删除。"))
            return
        box = QMessageBox(self)
        box.setWindowTitle(ui_text("删除模型"))
        box.setText(f"{ui_text('确定删除 ')}{self.code}{ui_text(' 模型的缓存文件（含未完成的下载残留）吗？')}")
        b_yes = box.addButton(ui_text("删除"), QMessageBox.DestructiveRole)
    ...
    def run(self):
        try:
            ...
            if (self.from_code, self.to_code) in set(ArgosEngine.installed_pairs()):
                self.finished_ok.emit(ui_text("该方向语言包已安装，无需重复下载"))
                return
            self.progress_text.emit(ui_text("正在获取语言包索引..."))
            packs = ArgosEngine.available_packages()
            match = [p for p in packs if p.from_code == self.from_code and p.to_code == self.to_code]
            if not match:
                self.failed.emit(
                    f"{ui_text('未找到 ')}{self.from_code} -> {self.to_code}{ui_text(' 的离线语言包（该方向暂无离线包，可改用在线引擎）')}")
                return
            pkg = match[0]
                ...
                if 5 <= pct < 95:
                    ...
                    if t:
                        self.progress_text.emit(
                            f"{ui_text('正在下载语言包 ')}{pkg.from_name} -> {pkg.to_name}"
                            f"{ui_text('（约 80MB，')}{pct}{ui_text('%，剩余约')}{t}）...")

            self.progress_text.emit(f"{ui_text('正在下载语言包 ')}{pkg.from_name} -> {pkg.to_name}{ui_text('（约 80MB）...')}")
            ArgosEngine.install(pkg, progress_cb=cb)
            self.progress_pct.emit(100)
            self.finished_ok.emit(f"{ui_text('语言包 ')}{pkg.from_name} -> {pkg.to_name}{ui_text(' 安装成功，可离线使用')}")
        except Exception as e:
            self.failed.emit(f"{ui_text('语言包安装失败: ')}{e}")
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, main):
        ...
        self.setWindowTitle(ui_text("设置 · LiveSubtitle"))
        self.setModal(False)
        self.resize(860, 580)
        ...
        self.dirty_hint = QLabel(ui_text("所有改动已保存"))
        self.dirty_hint.setObjectName("DirtyHint")
        bar_layout.addWidget(self.dirty_hint)
        bar_layout.addStretch()
        self.reset_button = QPushButton(ui_text("恢复默认"))
        ...
        self.apply_button = QPushButton(ui_text("保存并应用"))
        ...
        self.search_edit.setPlaceholderText(ui_text("🔍 搜索设置…"))
    ...
    def _on_search_changed(self, text):
        ...
        if hits:
            ...
            self.search_hint.setText(f"{ui_text('找到 ')}{len(hits)}{ui_text(' 项：')}{names}"
                                     + ("…" if len(hits) > 4 else ""))
        else:
            self.search_hint.setText(ui_text("没有匹配的设置项"))
    ...
    def _page_audio(self):
        page = self._page()
        self._section(page, ui_text("音频来源"))
        self.source_combo = QComboBox()
        self.source_combo.addItem(ui_text("系统声音（正在播放的内容）"), "system")
        self.source_combo.addItem(ui_text("麦克风"), "microphone")
        self._row(page, ui_text("音频来源"),
                  ui_text("看视频 / 听会议选「系统声音」，直接抓取电脑播放的一切声音；翻译别人说话选「麦克风」。"),
                  self.source_combo)
        ...
        self._row(page, ui_text("输入设备"),
                  ui_text("选择具体设备；更换耳机等设备后点「刷新」重新加载。蓝牙耳机的部分虚拟输出不支持抓取系统声音。"),
                  wrap)
        self.source_combo.currentIndexChanged.connect(self._on_source_changed)

        tip = QLabel(ui_text("开始翻译后，主窗口底部会显示实时音量条；音量条不动说明抓错了设备。"))
    ...
    def _page_asr(self):
        ...
        tips = {
            "tiny": ui_text("75MB · 延迟约 2s · 中文易误判，适合纯英文+老电脑"),
            "base": ui_text("145MB · 延迟约 2.5s · 中文较弱"),
            "small": ui_text("480MB · 延迟约 3s · 中文良好，推荐 4 核以上 CPU"),
            "medium": ui_text("1.5GB · 高精度 · 需较新多核 CPU 或 GPU（纯 CPU 慢机实时吃力，建议 small）"),
            "large-v3-turbo": ui_text("约1.6GB · 顶级精度 · 需 GPU 或高配 CPU（约 6s+）"),
        }
        ...
        self._row(page, ui_text("识别模型"),
                  ui_text("全部在本地运行，语音不出电脑。首次选择后自动下载模型（一次性），之后永久离线可用。中文内容建议 small。"),
                  model_wrap)

        self._section(page, ui_text("语言与计算"))
        ...
        self._std_rows(page, "asr", ui_text("语言与计算"),
                       keys=("hallucination_filter", "silero_vad", "lang_recheck",
                             ...
                             "stream_preview",
                             "prewarm_model"))
        self._section(page, ui_text("识别质量调优"))
        # v2.7.0（T3）：热词提示——事前纠正专名误听，与事后修正词典互补
        self.hotwords_edit = QLineEdit()
        self.hotwords_edit.setPlaceholderText(
            ui_text("人名/地名/产品名等，用逗号分隔，如：Noriega、OpenAI、霍尔木兹海峡（留空=关闭）"))
        self.hotwords_edit.setClearButtonEnabled(True)
        self._row(page, ui_text("热词提示"),
                  ui_text("识别前把这些专名提示给模型，专名命中率有改善可能（不保证，效果因内容而异）；"
        ...
        self.mishear_edit.setPlaceholderText(
            ui_text("每行一条误听修正：错误文本=正确文本\n例如：\nfeline=feel in\n芯片组=新奇点"))
        self.mishear_edit.setMaximumHeight(90)
        self._row(page, ui_text("误听修正词典"),
                  ui_text("对识别结果做精确替换（建议2/建议5 质量调优）；保存并应用后对后续字幕生效。"),
                  self.mishear_edit)
    ...
    def _page_translate(self):
        ...
        self.proxy_combo.addItem(ui_text("跟随系统代理（推荐）"), "system")
        self.proxy_combo.addItem(ui_text("手动指定"), "manual")
        self.proxy_combo.addItem(ui_text("不使用代理（直连）"), "none")
        self._row(page, ui_text("代理模式"),
                  ui_text("手动指定适合代理软件未开启系统代理、或需要端口转发的场景。"),
                  self.proxy_combo)
        ...
        self._row(page, ui_text("手动代理地址"),
                  ui_text("仅「手动指定」模式需要填写；支持 http/https/socks5（socks5 需安装 pysocks）。"),
                  proxy_wrap)
        ...
        self.argos_section_label = self._section(page, ui_text("离线语言包"))
        try:
            cleanup_temp_files()
        except Exception:
            pass
        self.argos_hint = QLabel(ui_text("选择 Argos 引擎后在此下载语言包（约 80MB / 包，一次下载永久离线使用）。"
        ...
        self.argos_download_button = QPushButton(ui_text("下载语言包"))
        ...
        self._section(page, ui_text("已安装语言包管理"))
        ...
        self.pack_remove_button = QPushButton(ui_text("卸载所选语言包"))
    ...
    def _page_general(self):
        ...
        self._section(page, ui_text("字幕记录"))
        self._std_rows(page, "general", ui_text("字幕记录"), keys=("max_history",))
        clear_btn = QPushButton(ui_text("清空翻译缓存"))
        self._row(page, ui_text("翻译缓存"),
                  ui_text("相同文本的翻译结果会本地缓存以加速显示；清空后下次重新翻译。不影响字幕记录。"),
                  clear_btn)
        clear_btn.clicked.connect(self._clear_cache)

        # v2.0.0：升级/跳过过向导的用户可随时重看三步引导
        wizard_btn = QPushButton(ui_text("重新运行首次向导"))
        self._row(page, ui_text("新手引导"),
                  ui_text("重新打开「音频源 → 模型 → 完成」三步向导（不会改动你的现有配置，确认步骤时可保持原样）。"),
                  wizard_btn)
        wizard_btn.clicked.connect(self._rerun_wizard)

        self._section(page, ui_text("全局热键"))
        self.hotkey_check = QCheckBox()
        self._row(page, ui_text("启用全局热键"),
                  ui_text("看视频/开会时无需切回本窗口，任何界面按热键即可开始/停止翻译。"),
                  self.hotkey_check)
        self.hotkey_edit = QKeySequenceEdit()
        self._row(page, ui_text("热键组合"),
                  ui_text("默认 Ctrl+Alt+S，可改键。需含 Ctrl/Alt/Shift/Win 至少一个修饰键，避免影响正常打字。"),
                  self.hotkey_edit)
        # v2.2.6：显隐悬浮条热键（可留空禁用）
        self.hotkey_overlay_edit = QKeySequenceEdit()
        self._row(page, ui_text("字幕面板显隐热键"),
                  ui_text("默认 Ctrl+Alt+O，任何界面按键即可显示/隐藏字幕面板；清空后禁用该热键。"),
        ...
        self._section(page, ui_text("存储位置"))
    ...
    def _rerun_wizard(self):
        ...
        if getattr(self, "_staged", None):
            ret = QMessageBox.question(
                self, ui_text("未保存的修改"),
                ui_text("当前有未保存的设置修改，运行向导会丢弃这些修改。\n\n"
                "要先「保存并应用」再运行向导吗？"),
    ...
    def _change_storage_root(self):
        ...
        if getattr(self, "_storage_worker", None) and self._storage_worker.isRunning():
            QMessageBox.warning(self, ui_text("正在迁移"),
                                ui_text("数据迁移正在进行中，请等待完成后再操作。"))
            return
        if getattr(self.main, "running", False):
            QMessageBox.warning(self, ui_text("无法更改"),
                                ui_text("翻译运行中不能迁移数据，请先停止翻译。"))
            return
        new = QFileDialog.getExistingDirectory(
            self, ui_text("选择新的数据根目录（模型/语言包将迁移到此目录下）"),
    ...
    def _manage_models(self):
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle(ui_text("识别模型管理"))
        dlg.resize(500, 360)
        v = QVBoxLayout(dlg)
        hint = QLabel(ui_text("双击模型查看介绍并可下载/删除；模型只在本机运行，删除后下次选择会自动重新下载。"))
        ...
        def status_text(code):
            ...
            base = {
                "missing": ui_text("未下载（首次选择时自动下载，也可手动下载）"),
                "partial": f"{ui_text('下载不完整 · 残留 ')}{mb:.0f}{ui_text(' MB（可删除或续传）')}",
                "full": f"{ui_text('已下载 · ')}{mb:.0f} MB",
            }[state]
            if code == current:
                base += ui_text(" · 当前使用")
            return base
        ...
        rm_btn = QPushButton(ui_text("删除所选模型"))
        ...
        def do_remove():
            ...
            if code == current and getattr(self.main, "running", False):
                QMessageBox.warning(dlg, ui_text("无法删除"),
                                    ui_text("该模型正在使用中，请先停止翻译再删除。"))
                return
            box = QMessageBox(dlg)
            box.setWindowTitle(ui_text("删除模型"))
            box.setText(f"{ui_text('确定删除 ')}{code}{ui_text(' 模型的缓存文件（含未完成的下载残留）吗？')}")
            b_yes = box.addButton(ui_text("删除"), QMessageBox.DestructiveRole)
            box.addButton(ui_text("取消"), QMessageBox.RejectRole)
            box.exec()
            if box.clickedButton() != b_yes:
    ...
    def _show_gpu_guidance(self):
        dlg = QDialog(self)
        dlg.setWindowTitle(ui_text("GPU / CUDA 环境检测"))
        dlg.resize(620, 480)
        v = QVBoxLayout(dlg)
        title = QLabel(ui_text("检测结果"))
        title.setObjectName("SettingTitle")
        v.addWidget(title)
        # v2.0.1：检测挪到后台线程——nvidia-smi 子进程（超时 8s）+ ctranslate2
        # CUDA 枚举此前在 GUI 线程同步执行，驱动异常时界面冻结 8 秒以上
        summary = QLabel(ui_text("正在检测（显卡 / 驱动 / CUDA 环境，最长约 10 秒）…"))
        summary.setObjectName("SettingDesc")
        summary.setWordWrap(True)
        v.addWidget(summary)
        v.addWidget(self._sep())
        t = QLabel(ui_text("配置教程与注意事项"))
        t.setObjectName("SettingTitle")
        v.addWidget(t)
        body = QLabel(ui_text("检测完成后显示。"))
        body.setObjectName("SettingDesc")
        body.setWordWrap(True)
        v.addWidget(body, 1)
        btn_row = QHBoxLayout()
        btn_row.addStretch()
        install_btn = QPushButton(ui_text("一键安装 CUDA 推理运行时"))
        ...
        def _apply(info):
            ...
            state_txt = {"cuda": ui_text("已装 CUDA 版（运行时就绪）"),
            ...
            if not info["frozen"] and info["nvidia_gpu"] and torch_state != "cuda":
                ...
                install_btn.setText(ui_text("升级为 CUDA 版运行时") if torch_state == "cpu"
                                    else ui_text("一键安装 CUDA 推理运行时"))
            else:
                install_btn.setVisible(False)
    ...
    def _install_cuda_torch(self, parent_dlg, btn):
        ...
        box = QMessageBox(self)
        box.setWindowTitle(ui_text("确认安装"))
        box.setText(ui_text("将从 PyTorch 官方源下载并安装 CUDA 12.1 版 PyTorch（约 2GB+）。\n"
                    "安装期间请保持网络与电源稳定，完成后需重启本程序生效。"))
        b_go = box.addButton(ui_text("开始安装"), QMessageBox.AcceptRole)
        box.addButton(ui_text("取消"), QMessageBox.RejectRole)
        box.exec()
        if box.clickedButton() != b_go:
            return
        btn.setEnabled(False)
        btn.setText(ui_text("正在安装…"))
    ...
    def _on_cuda_done(self, ok, msg, btn):
        btn.setEnabled(True)
        btn.setText(ui_text("一键安装 CUDA 推理运行时"))
        if ok:
            QMessageBox.information(self, ui_text("安装完成"),
                                    ui_text("CUDA 推理运行时（cuDNN/cuBLAS）已安装。\n"
    ...
    def _page_about(self):
        ...
        ver_label = QLabel(f"{ui_text('实时字幕翻译 · v')}{APP_VERSION}")
        ...
        self._section(page, ui_text("软件更新"))
        ...
        self._section(page, ui_text("链接"))
        self._about_link(page, ui_text("更新日志"), ui_text("查看版本历史与改进内容"), DOCS_URL)
        self._about_link(page, ui_text("问题反馈 / 源码仓库"), ui_text("提交 Issue 或 Fork 贡献"), REPO_URL)
    ...
    def _check_app_update(self):
        ...
        self.app_update_status.setText(ui_text("正在检查 GitHub Releases ..."))
    ...
    def _on_app_check_done(self, latest, err):
        ...
        if err:
            self.app_update_status.setText(f"{ui_text('✗ 检查失败：')}{err}")
            return
        if not latest:
            self.app_update_status.setText(ui_text("未获取到版本信息。"))
            return
        if _version_tuple(latest) > _version_tuple(APP_VERSION):
            self.app_update_status.setText(
                f"{ui_text('🎉 发现新版本 v')}{latest}{ui_text('！可到 Releases 页下载安装包更新（保留用户数据）。')}")
            QDesktopServices.openUrl(QUrl(f"{REPO_URL}/releases"))
        else:
            self.app_update_status.setText(f"{ui_text('✓ 已是最新版本（最新 v')}{latest}）。")

    def _check_model_update(self):
        self.model_update_btn.setEnabled(False)
        self.model_update_status.setText(ui_text("正在检查 HuggingFace 模型版本 ..."))
    ...
    def _on_model_check_done(self, rev, err):
        ...
        if err:
            self.model_update_status.setText(f"{ui_text('✗ 检查失败：')}{err}")
            return
        self.model_update_status.setText(
            f"{ui_text('✓ 当前使用 ')}{cur}{ui_text(' 模型，线上最新修订 ')}{rev}{ui_text('。模型随首次下载固定，重装才会更新。')}")

    def _check_pack_update(self):
        self.pack_update_btn.setEnabled(False)
        self.pack_update_status.setText(ui_text("正在检查 Argos 语言包索引 ..."))
    ...
    def _on_pack_check_done(self, packs, err):
        ...
        if err:
            self.pack_update_status.setText(f"{ui_text('✗ 检查失败：')}{err}")
            return
        try:
        ...
        self.pack_update_status.setText(
            f"{ui_text('✓ 索引可访问，共 ')}{len(packs)}{ui_text(' 个可用语言包；本机已安装 ')}{len(installed)}{ui_text(' 个')}"
            + (ui_text("（索引与本地均在线，无需更新）") if installed else "。"))
    ...
    def _mark_dirty(self):
        ...
        self.dirty_hint.setText(ui_text("有未保存的修改") if dirty else ui_text("所有改动已保存"))
    ...
    def _apply_staged(self):
        ...
        if restart_keys:
            if self.main.running:
                ...
                self.dirty_hint.setText(ui_text("已保存并应用 · 管线已重启"))
            else:
    ...
    def _reset_defaults(self, confirm=True):
        ...
        if confirm and QMessageBox.question(
                self, ui_text("恢复默认"),
                ui_text("把**全部**设置项恢复为出厂默认（识别模型、引擎、热键、面板外观等都会变）。\n"
        ...
        self.hotkey_status.setText(ui_text("已暂存默认值，点「保存并应用」生效"))
        self._mark_dirty()
        self._preview_overlay_style()
    ...
    def _confirm_discard(self):
        if not self._staged:
            return True
        box = QMessageBox(self)
        box.setWindowTitle(ui_text("未保存的修改"))
        box.setText(ui_text("有未保存的设置修改，确定放弃并关闭吗？"))
        b_discard = box.addButton(ui_text("放弃修改"), QMessageBox.DestructiveRole)
        box.addButton(ui_text("返回继续编辑"), QMessageBox.RejectRole)
        box.exec()
    ...
    def _on_device_changed(self):
        ...
        self._stage("device_name", combo.currentText().replace(ui_text(" [系统声音]"), ""))
        ...
        if not self.proxy_url_edit.text().strip():
            if manual:
                ...
                self.proxy_url_edit.setPlaceholderText(
                    f"{ui_text('如 http://127.0.0.1:10808（检测到系统代理：')}{sys_url}）"
                    if sys_url else ui_text("如 http://127.0.0.1:10808"))
            else:
                self.proxy_url_edit.setPlaceholderText(ui_text("仅「手动指定」模式需要填写"))

    def _test_proxy(self):
        self.proxy_test_button.setEnabled(False)
        self.proxy_status_label.setText(ui_text("正在探测 Google 免费翻译通道（最长 4 秒）..."))
    ...
    def _pick_color(self, which):
        ...
        color = QColorDialog.getColor(cur, self, ui_text("选择颜色"))
    ...
    def _clear_cache(self):
        _cache.clear()
        QMessageBox.information(self, ui_text("完成"), ui_text("翻译缓存已清空。"))
    ...
    def _load_devices(self):
        ...
        self._device_map = {}
        for d in devices:
            tag = ui_text(" [系统声音]") if d.get("loopback") else ""
            label = f"{d['name']}{tag}"
        ...
        if source_type == "system":
            ...
            # （麦克风分支一直有默认项，两分支行为分叉）
            self.device_combo.insertItem(0, ui_text("默认输出设备（自动）"), -1)
        else:
            self.device_combo.addItem(ui_text("默认麦克风（自动）"), -1)
        ...
        if preferred_name:
            for i in range(self.device_combo.count()):
                if self.device_combo.itemText(i).replace(ui_text(" [系统声音]"), "") == preferred_name:
                    idx = i
                    break
        if idx < 0:
    ...
    def _remove_selected_pack(self):
        ...
        try:
            ...
            QMessageBox.warning(self, ui_text("卸载失败"), f"{ui_text('删除目录时出错：')}{e}")
            return
        if removed:
            ...
            self._refresh_packs_list()
            self._refresh_argos_section()
            QMessageBox.information(self, ui_text("完成"), f"{ui_text('已卸载 ')}{name}{ui_text('（释放 ')}{freed}）。")
        else:
            self._refresh_packs_list()
    ...
    def _download_argos(self):
        ...
        self.argos_hint.setText(f"{ui_text('正在安装离线语言包: ')}{code} -> {tgt_name}")
