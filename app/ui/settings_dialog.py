...
_STD_ROW_ITEMS = {
    ...
    "segment_cap": [("2.5 秒（刷新最密，句子最易被腰斩）", 2.5),
                    ("3 秒", 3.0),
                    ("4 秒（默认，= 榨干模式档）", 4.0),
                    ("6 秒（低延迟模式旧默认）", 6.0),
                    ("10 秒（整句优先，明显更慢）", 10.0),
...
_STD_ROWS = [
    ...
    {"key": "spec_translate", "attr": "spec_translate_check", "page": "asr", "section": "语言与计算",
     "kind": "check", "title": "推测式增量翻译（治译文迟到）",
     "desc": "识别碎片一到达就立刻翻译「目前攒到的文本」并上屏，下一片到达再送更长版本，"
