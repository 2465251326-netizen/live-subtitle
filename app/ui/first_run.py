...
class FirstRunWizard(QDialog):
    ...
    def _page_done(self):
        ...
        if hk and bool(self.main.config.get("hotkey_enabled")):
            # v2.7.4（QA-01 活体实锤）：v2.5.3 已把该开关改名去掉"（置顶）"
            # （置顶语义归面板 📌），向导此处一直停留在旧名——文案撒谎的教唆犯
            ov_line = f"· 字幕面板：勾选「启用字幕面板」，或任何界面按 {hk} 直接显隐\n"
        else:
            ov_line = ("· 字幕面板：勾选「启用字幕面板」开启；是否压在其他窗口之上，"
                       "用面板工具条的 📌 控制；显隐组合热键可在「设置-通用」开启并设置\n")
        tips = QLabel(
