...
1. **选音频源**：在设置面板「音频输入」中选择。看视频 / 听会议选「系统声音」，翻译别人对你说话选「麦克风」
...
| 按钮 | 作用 |
|------|------|
| 设置 | 展开 / 收起左侧设置抽屉（收起后字幕区占满全窗） |
| 使用说明 | 打开浏览器跳转本页 |
| 开始翻译 / 停止 | 启动或停止实时字幕 |

状态栏显示当前使用的翻译引擎、识别语言与本次会话字幕条数。

### 设置抽屉（左侧面板）

点击顶栏「设置」按钮可展开或收起，收起状态会自动记忆。

**音频输入**

- **音频源**：「系统声音」采集电脑播放的一切声音（推荐看视频用）；「麦克风」采集外部人声
- **设备**：选择具体输入 / 输出设备；更换耳机等设备后点旁边的刷新按钮重新加载
- **音量条**：实时显示采集音量。开始翻译后音量条不动说明抓错了设备
...
- 选择 Argos 引擎时，面板出现语言包下载区，按「识别语言 → 目标语言」方向下载对应包
...
| 界面 | PySide6（Qt6） | 深色玻璃风格、抽屉设置、悬浮字幕、系统托盘 |
