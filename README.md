...
- 🌐 **界面语言可切换**：简体中文 / English（「设置-通用」→ 界面语言，改完重启生效）；只换软件自身的菜单与说明，不影响字幕的翻译语言
...
- **界面语言**：简体中文 / English 两档。改完**需重启软件**才生效（界面文字在启动时一次性载入），保存后状态行会如实提示"需重启后生效"。此项只切换软件自身的菜单、按钮与说明文字，**不影响字幕翻译成什么语言**——那由「翻译」页的「目标语言」决定。目前只做中英两档：日/韩/泰等对本工具受众属小语种，每加一档就多一份永远翻译不完的词典
...
- **历史滚动**：「列表历史」正文是原文+译文成对的历史滚动区；「上下双语」两栏各自逐句累积。**向上滚动即暂停自动跟随，且会一直保持暂停**——新句到达不会把你正在读的那行顶回去，工具条上出现「↓ 最新 N」计数按钮，点它或自己滚回底部即恢复跟随（v2.22.0 起两种布局同一套契约）。
  「上下双语」两栏**始终不显示滚动条**（滚轮回看照常）；「列表历史」只在内容超过面板高度上限（屏幕 55%）时才浮出滚动条，因为那条路要装下整场历史
- **状态行**：工具条下方一行，只在**有话说**的时候出现——正在加载 / 正在下载识别模型（带百分比）、系统静音、输入信号弱、识别跟不上（积压丢段）、翻译连续失败、模型加载失败、已停止。v2.22.0 前这行字挤在工具条 8 颗按钮中间，实测只分到 67px（英文界面 7px），等于从来没被看见过；现在它独占一行，没话可说时整行收起不占正文高度
- **不重复上屏**：识别偶尔会把刚说过的一句连着尾巴再转一遍（滑窗重播），屏上就出现同一句两行、各带一份措辞不同的译文。v2.23.0 起面板与主窗共用同一把"跨行回声"判据，v2.23.2 把它收成一个函数（逐词包含 / 首尾错位 / 开头同源且词序重合 ≥85%，并排除"只差等长一小段"的两句真话），并把**处置**从"这一拍丢掉"改成"**就地覆盖那一行，更完整的版本胜出**"。同时流式草稿（句子还在长的时候）不再有另起一行的资格——实机样本里 `The.`、`of our`、`so that...` 这类垃圾行与半句重播行全是它开的；草稿也不再把下一句接进上一行（追加部分大写开头 + 前文停在句末标点 = 换句）。四条护栏保证它不吞真话：只跟已出译文的历史行比、短句复读（"Great." 说两遍）不去重、开头不同的同构两句不去重、等长中段替换（"第 7 条"/"第 8 条"）算两句话。**已知残留**：上一行的尾巴正好是下一句的开头时，仍会各留一行（消它需要改写已经上屏那一行的尾部，风险是裁掉真话，故未做）
