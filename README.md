...
- **双语面板历史区**（默认关＝滚动字幕墙）：关闭时整个面板是一面滚动的字幕墙——每句说完留在屏上成对驻留（原文+译文），当前句在最底部实时生长，满屏自动上滚，滚轮上滑回看不被打断，**句子不会从悬浮窗里消失**；开启＝经典上下双语（当前句大字区 + 顶部历史块，最多 30 对，可拖分割线分配高度）。两种形态都不影响主窗口与导出文件的完整记录，面板 ⋯ 菜单随手可切、重启保持
