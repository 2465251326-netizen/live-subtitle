...
- **面板布局**：「列表历史」= 原文+译文成对的历史滚动区（默认）；「上下双语」= 豆包式实时翻译——上半是随识别流式生长的原文（淡色小字），下半是加粗大字译文，配合推测式增量翻译几乎无等待，说完即换下一句。面板 ⋯ 菜单可随时互切
- **流式原文**（上下双语增强，默认开）：每 0.9 秒把最近 4 秒音频重识别一次，主持人讲到哪、原文跟到哪，不再等分段周期——连续语流（新闻口播）下原文延迟约 1 秒。仅 GPU 模式实际启用（CPU 自动停用）
