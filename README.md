...
- 🪟 **悬浮字幕面板**：两种布局任选——「列表历史」（原文+译文成对滚动回看）与「上下双语」（豆包风：上原文 / 可拖分割线 / 下译文大字，当前句聚焦）；启动即常驻、置顶、可拖可缩、外观全可调
...
- **面板布局**：「列表历史」= 原文+译文成对的历史滚动区（默认，可回看整场）；「上下双语」= 豆包式实时翻译——整个面板给**正在说的这一句**：上半原文随识别流式生长（淡色），下半加粗大字译文随推测式翻译就地更新，几乎无等待；**中间那条分割线可以上下拖**，原文长句想留多大就拖多大，两栏各自能滚轮回看。双语布局说完即换下一句，回看整场请用「列表历史」（主窗卡片与导出文件永远是全的）。面板 ⋯ 菜单可随时互切
