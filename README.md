...
- ⚡ **推测式增量翻译**：译文跟着识别碎片原地生长上屏，不再干等整句攒完（离线引擎下真机实测译文出现从 5.1 秒缩短到 0.1 秒）
- 🪟 **悬浮字幕面板**：两种布局任选——「列表历史」（原文+译文成对滚动回看）与「上下双语」（豆包风：上原文下译文大字，当前句聚焦）；启动即常驻、置顶、可拖可缩、外观全可调
...
- **启用字幕面板**：面板当前是否显示。**启动即常驻**——在此取消勾选、或用热键 / 面板 ✕ 隐藏，只在本次运行内有效，下次打开软件会自动恢复显示
- **同时显示原文**：面板内译文上方同时保留原文（面板工具条上也可一键开关）
- **面板样式**：字号 / 文字颜色 / 背景颜色 / 背景不透明度，全部即时生效
- **面板布局**：「列表历史」= 原文+译文成对的历史滚动区（默认）；「上下双语」= 豆包式实时翻译——整个面板给**正在说的这一句**：上半原文随识别流式生长（淡色），下半加粗大字译文随推测式翻译就地更新，几乎无等待。面板 ⋯ 菜单可随时互切
- **双语面板历史区**（默认开）：「上下双语」面板上半部保留历史区，每句翻译完成自动沉入（原文小灰+译文小白成对，最多 30 对），说过的句子在悬浮窗里不消失、滚轮回看；高度按内容自动收缩、最多占面板一半，当前句区保底不被挤没，两区之间可拖分割线自由分配。关＝当前句独占面板（说过的句子仍在主窗口与导出文件里完整保留）。面板 ⋯ 菜单里也有同名开关，随手可切、重启保持
...
- **启动即常驻**：软件一打开面板即显示，无需任何唤出动作；热键（默认 `Ctrl + Alt + O`）随时隐藏/再显示，隐藏只在当次运行内有效（下次启动恢复常驻）
- **拖动移动**：按住顶部工具条拖动，位置自动记忆
- **调整大小**：右缘拖宽、底缘拉高
- **历史滚动**：正文是原文+译文成对的历史滚动区；向上滚动即暂停自动跟随，回到底部恢复
- **📌 钉住**：工具条 📌 钉住=悬浮于其他程序之上，取消=可以点穿去操作底下窗口
- **收起态**：工具条 ⌄ 可把面板收成一条精简字幕行，再点 ⌃ 展开
